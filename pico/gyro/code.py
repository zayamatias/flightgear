...
def rotate(motor,currval,newval,corrector=False):
    ...
    #print (currval,newval)
    if motor['stepper']:
        if not motor['warp']:
            diff=currval-newval
        else:
            angular_difference = newval - currval    
            # Normalize the angular difference to be between -180 and 180 degrees
            angular_difference = (angular_difference + 180) % 360 - 180
            # Determine the shortest path
            if angular_difference > 0:
                diff = -1*(abs(angular_difference))
            elif angular_difference < 0:
                diff = (abs(angular_difference))
            else:
                diff = 0
        ...
        if diff >0:
            mydir = motor['forward']
            myotherdir = motors[1]['backward']
        else:
            mydir = motor['backward']
            myotherdir = motors[1]['forward']
        for n in range(1,steps):
            stepper_move(mystepper,mydir)
            if motor['name']=='heading':
                #print ('moving HDG BUG TOO')
                stepper_move(motors[1]['stepper'],myotherdir)
...
val['motor']=1
val['corrector']=False
val['avoidzero']=False ## If this is True then it does not starts going back from 127 to zero at startup
printout.append(val)
val = dict()
## CORRECTED HEADING BUG
val['name']='CORRHDGBUG'
val['digits']=[9,10,11]
val['currval']=0
val['motor']=1
